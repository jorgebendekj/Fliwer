...
class EmailAuth extends Component {
            ...
            <View style={{width: "100%", alignItems: "center"}}>
                <View style={[{width: "100%", marginTop: 0}, Platform.OS === 'web'? {alignItems: "center"} : {}]}>
                    <Text style={[FliwerStyles.littleTextStyle, {textAlign: "center"}]}>{this.props.firstTitle? this.props.firstTitle :
                            ...
                            this.props.actions.translate.get('emailAuth_enter_email')
                    }</Text>
                </View>
                  <FliwerTextInput
                      ...
                      disabled={true/*this.props.fastVerification*/}
                  />
    ...
    async sendEMail() {
        ...
        this.props.actions.sessionActions.requestEmailVerification(email).then((confirmationResult) => {
            ...
            that.setState({confirmationResult: confirmationResult, validEmail: that.state.smail});
        }).catch(function (error) {
            console.log("Email not sent!. Err: ", error);
            that.props.setLoading(false);
            that.props.setInfoModalVisible(true, false, "tomato",
...
//Connect everything
export default connect(mapStateToProps, mapDispatchToProps,null,{ forwardRef: true })(mediaConnect(style, EmailAuth));