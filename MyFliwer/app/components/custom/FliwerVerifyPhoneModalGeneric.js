...
class FliwerVerifyPhoneModalGeneric extends Component {
    ...
    render() {
        var {animationType, visible, onCancel, country, title, subtitle, smsInfo, cancelText} = this.props;
                ...
                <Modal animationType={animationType ? animationType : "fade"} loadingModal={this.state.loading} inStyle={this.style.modalIn} visible={visible} onClose={() => {
                    ...
                    <View style={[this.style.modalView, {maxHeight: 620}]}>
                        <ScrollView scrollEventThrottle={1000} style={[{width: "100%"}, Platform.OS == "web"? {height: "100%"} : {}]} contentContainerStyle={{justifyContent: "space-between"}}>
                            ...
                            <Text style={this.style.modalViewTitle}>{title?title:"Verifica esta acción con un SMS"}</Text>
                            <Text style={this.style.modalViewSubtitle}>{subtitle?subtitle:"La acción que quieres realizar requiere una comprobación con SMS."}</Text>
                            <View style={[this.style.form, Platform.OS !== 'web'? {paddingLeft: 10, paddingRight: 10} : {}]}>
                                ...
                                <PhoneAuth
                                    ...
                                    firstTitle={smsInfo?smsInfo:"Introduce tu número de teléfono"}
                        ...
                        <View style={this.style.modalButtonContainer}>
                            <TouchableOpacity style={[this.style.modalButton, this.style.modalButton2]} onMouseEnter={this.hoverIn('modalButton2')} onMouseLeave={this.hoverOut('modalButton2')}
                                ...
                                <Text style={[this.style.modalButtonText, this.style.modalButtonTextYes]}>{cancelText?cancelText:"Cancelar acción"}</Text>
                            </TouchableOpacity>
                        </View>
    ...
    onValidatePhoneAuth(verifiedPhone, verifiedCode, verificationId)
    {
        ...
        return new Promise((resolve, reject) => {
            ...
            if(onSuccess)onSuccess({verifiedPhone:verifiedPhone,verifiedCode:verifiedCode,verificationId:verificationId,verificationTime:verificationTime})
            resolve();
        });
...
//Connect everything
export default connect(mapStateToProps, mapDispatchToProps)(mediaConnect(style, FliwerVerifyPhoneModalGeneric));
