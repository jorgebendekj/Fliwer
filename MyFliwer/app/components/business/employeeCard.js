...
class EmployeeCard extends Component {
    ...
    renderTopText() {
        var card = "";
        if (this.state.employee.first_name || this.state.employee.last_name)
            card = card + this.state.employee.first_name.toUpperCase() + ' ' + this.state.employee.last_name.toUpperCase()
        else
            card = '--'
    ...
    renderCardFront() {
        ...
        if (this.state.goWorkerDetails) {
            card.push(<Redirect push to={`/app/workers/details/${this.state.goWorkerDetails}`} />)
        ...
        if (typeof this.props.onPressAdd === 'function') {
            //Create employee button card
            card.push(
                <TouchableOpacity key={"cardFront1_" + this.state.employee.idUser} style={{ width:"95%", alignSelf: "center", height: 110, borderRadius: 7, alignItems: "center", justifyContent: "center", backgroundColor: FliwerColors.secondary.white, marginTop: 10, opacity: Platform.OS == "android" ? 0.6 : 0.4 }}
                    onPress={() => {
        ...
        card.push(
            <TouchableOpacity
                key={"cardFront2_" + this.state.employee.idUser}
                style={{
                    width: "95%",
                    height: 110,
                    backgroundColor: selectedWorker == this.state.employee.user_id
                        ? CurrentTheme.lighterCardColor
                        : CurrentTheme.darkerCardColor,
                    borderRadius: 7,
                    borderWidth: selectedWorker == this.state.employee.user_id ? 2 : 0,
                    borderColor: CurrentTheme.selectedColor,
                    marginTop: 10,
                    alignSelf: "center"
                }}
                onPress={() => {
                    this.setState({ goWorkerDetails: this.state.employee.user_id });
                }}
            >
                <View
                    style={{
                ...
                >
                    <View style={{ width: 90, flexShrink: 0 }}>
                        <Image
                            ...
                            source={{ uri: this.state.employee.image }}
                            resizeMode={"cover"}
                        />
                    </View>
                    ...
                    >
                        ...
                        {/* Position */}
                        {this.state.employee.businessPosition && (
                            <Text
                                style={[
                        ...
                        {/* Botón de licencias comentado */}
                        {/*
      {this.state.employee.license && Object.keys(this.state.employee.license).length > 0 && (
        <FliwerCalmButton
          containerStyle={[this.style.buttonContainer, this.style.buttonContainer2]}
          buttonStyle={this.style.buttonAccess}
          onPress={() => {
            this.card.current._toggleCard();
          }}
          textStyle={this.style.buttonTextIn}
          text={"Licencias"}
        />
      )}
      */}
        ...
        if (this.state.employee.pending || this.state.employee.petition) {
            if (this.state.employee.pending)
                var text = this.props.actions.translate.get('Gardener_pending');
            if (this.state.employee.expired)
                text = this.props.actions.translate.get('FliwerButtonProgramDetail_expired');
            card.push(
                <TouchableOpacity key={"cardFront8_" + this.state.employee.idUser} style={[this.style.layerEdit]} onPress={() => {
                    if (this.props.modalDelete) {
                        this.props.modalDelete(true, this.state.employee.idUser);
                    }
                }} >
...
//Connect everything
export default connect(mapStateToProps, mapDispatchToProps, null, { forwardRef: true })(mediaConnect(style, EmployeeCard));
