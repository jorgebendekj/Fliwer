...
import AppStoreCard from './appStoreCard.js'
...
class businessAPPStore extends Component {
    constructor(props) {
        super(props);
    ...
    componentDidMount() {
        ...
        this.props.actions.sessionActions.getApps().then((response) => {
    ...
    render() {
        if (this.state.goADDusers) {
            return (<Redirect push to={"/business/providers/add"} />)
            ...
            return (
                <ImageBackground  style={{backgroundColor:CurrentTheme.secondaryColor}}  resizeMode={"cover"} loading={loading}>
                    ...
                    <FliwerDeleteModal
                        ...
                        onClose={() => {
                            this.setModalVisible(false, this.state.idProduct);
                        }}
                        onConfirm={async (password) => {
                            await this.deleteProduct(password)
                        }}
                        title={"¿Quieres eliminar este producto?"/*this.props.actions.translate.get('masterVC_remove_user_gardener')*/}
                        hiddeText={true}
                        password={false}
    ...
    renderProducts() {
        ...
        var apps=MenuTheme.allApps;
        //productList=productList.filter(user=>user.clientObject && user.clientObject.isProvider)
       
        for (var index in apps) {
            indents.push(<AppStoreCard key={"appStore_card_"+apps[index].app} 
                idApp={apps[index].app} 
            />);
        }
    ...
    deleteProduct() {
        ...
        this.props.actions.invoiceActions.deleteProduct(this.state.idProduct).then((response) => {
            this.setState({loading: false,modalVisible: false, idProduct: null});
        }, (error) => {
            this.setState({ loading: false,modalVisible: false, idProduct: null });
            if (err && err.reason)
                toast.error(err.reason);
    ...
    setModalVisible(visible, idProduct) {
        this.setState({modalVisible: visible,modalClientObjectVisible:false, idProduct: idProduct});
    }
    
    setModalClientObjectVisible(visible, idProduct, product) {
        
        this.setState({modalClientObjectVisible: visible,modalVisible:false, idUser: idProduct, productInfo: product});
    }
...
export default connect(mapStateToProps, mapDispatchToProps)(mediaConnect(styles, businessAPPStore));
